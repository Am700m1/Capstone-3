...
public class ContractService {
    ...
    public void addContract(ContractDTOIn contractDTOIn, Integer reservation_id) {
        ...
        if (!contractDTOIn.getEndDate().isAfter(contractDTOIn.getStartDate())) {
            throw new ApiException("Contract end date must be after start date");
        }
        if (contractDTOIn.getStartDate().isBefore(reservation.getReservationDate())) {
            throw new ApiException(
                    "Contract start date cannot be before the reservation date");
        }
